...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    # if epoch_num % 2 == 0:
    #     if args.separate_branch:
    #         recalls1, recalls_str1 = test.test(
    #             args, test_ds1, model, model_db=model_db, test_method=args.test_method)
            
    #         recalls2, recalls_str2 = test.test(
    #             args, test_ds2, model, model_db=model_db, test_method=args.test_method)
    #     else:
    #         recalls1, recalls_str1 = test.test(
    #             args, test_ds1, model, model_db=model, test_method=args.test_method)
    #         recalls2, recalls_str2 = test.test(
    #             args, test_ds2, model, model_db=model, test_method=args.test_method)
        ...
        # logging.info(f"Recalls on test 0.8 set for {epoch_num} epoch: {recalls_str1}")
        # logging.info(f"Recalls on test 0.6 set for {epoch_num} epoch: {recalls_str2}")


    # If recall@5 did not improve for "many" epochs, stop training
    if is_best:
        logging.info(
            f"Improved: previous best R@1 = {best_r1:.1f}, current R@1 = {recalls[0]:.1f}"
        )
        best_r1 = recalls[0]
        not_improved_num = 0
    else:
