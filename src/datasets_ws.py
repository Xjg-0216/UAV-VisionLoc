...
class BaseDataset(data.Dataset):
    ...
    def __init__(
        self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", loading_queries=True, che=1
    ):
        super().__init__()
        self.args = args
        ...
        # self.database_folder_h5_path = join(
        #     datasets_folder, dataset_name, "database1022_sample3.h5"
        # )
        if loading_queries:
            if che == 1:
                ...
                self.queries_folder_h5_path = join(
                    datasets_folder, dataset_name, split + "0.8" + "_queries.h5"
                )
            elif che == 0.6:
                self.queries_folder_h5_path = join(
                    datasets_folder, dataset_name, split + "0.6" + "_queries.h5"
                )
        else:
        ...
        centerResize_transform = transforms.Compose(
            [
                # transforms.RandomAffine(degrees=0, scale=(1.0, 1.5)),  # 随机放大
                transforms.CenterCrop(size=self.resize),  # 中心裁剪
            ]
        )
        self.query_transform = transforms.Compose(
            [
...
def distort(img_path, do_process=True):
    ...
    #     bottom=(512 - new_height) // 2,
    #     left=(512 - new_width) // 2,
    #     right=(512 - new_width) // 2,
...
class SingleBaseDataset(data.Dataset):
    """Dataset with images from database and queries, used for inference (testing and building cache)."""

    def __init__(
        self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", loading_queries=False, use_h5_for_queries=True
    ):
        super().__init__()
        self.args = args
        self.dataset_name = dataset_name
        ...
        self.use_h5_for_queries = use_h5_for_queries
        self.resize = args.resize
        self.test_method = args.test_method

        # Redirect datafolder path to h5
        if loading_queries:
            if use_h5_for_queries:
                self.data_folder_h5_path = join(
                    datasets_folder, dataset_name, split + "_queries.h5"
                )
                if not os.path.exists(self.data_folder_h5_path):
                    raise FileNotFoundError(f"HDF5 file {self.data_folder_h5_path} does not exist.")
                # 打开 h5 文件
                data_folder_h5_df = h5py.File(self.data_folder_h5_path, "r")
                self.data_name_dict = {}
                for index, data_image_name in enumerate(data_folder_h5_df["image_name"]):
                    self.data_name_dict[data_image_name.decode("UTF-8")] = index
                self.data_paths = list(self.data_name_dict)
                self.data_utms = np.array(
                    [(path.split("@")[1], path.split("@")[2])
                     for path in self.data_paths]
                ).astype(np.float)
                self.images_paths = list(self.data_paths)
                self.data_num = len(self.data_paths)
                self.data_folder_h5_df = None
                data_folder_h5_df.close()

            else:  # 从文件夹中加载 queries
                self.data_folder_path = join(datasets_folder, dataset_name, split)
                if not os.path.exists(self.data_folder_path):
                    raise FileNotFoundError(f"Folder {self.data_folder_path} does not exist.")
                # 获取文件夹中所有的图像文件路径
                self.images_paths = [join(self.data_folder_path, f) for f in os.listdir(self.data_folder_path) if f.endswith(('.jpg', '.png'))]
                self.data_utms = np.array(
                    [(path.split("@")[1], path.split("@")[2])
                    for path in self.images_paths]
                ).astype(np.float)

                self.data_num = len(self.images_paths)
        else:

            self.data_folder_h5_path = join(
                datasets_folder, dataset_name, split + "test_database.h5"
            )
            if not os.path.exists(self.data_folder_h5_path):
                raise FileNotFoundError(f"HDF5 file {self.data_folder_h5_path} does not exist.")
            # 打开 h5 文件
        ...
        centerResize_transform = transforms.Compose(
            [
                # transforms.RandomAffine(degrees=0, scale=(1.0, 1.3)),  # 随机放大
                transforms.CenterCrop(size=self.resize),  # 中心裁剪
            ]
        )
        self.query_transform = transforms.Compose(
    ...
    def __getitem__(self, index):
        # Init
        # if self.data_folder_h5_df is None:
        #     self.data_folder_h5_df = h5py.File(
        #         self.data_folder_h5_path, "r")
        ...
        if self.loading_queries:
            if self.use_h5_for_queries:
                if self.data_folder_h5_df is None:
                    self.data_folder_h5_df = h5py.File(self.data_folder_h5_path, "r")
                if self.args.G_contrast:
                    img = self.query_transform(
                        transforms.functional.adjust_contrast(self._find_img_in_h5(index), contrast_factor=3))
                else:
    ...
    def _find_img_in_h5(self, index, database_queries_split=None):
        ...
        img = Image.fromarray(
            self.data_folder_h5_df["image_data"][
                self.data_name_dict[image_name]
            ]
        )
        return img
    ...
    def __repr__(self):
        return f"< {self.__class__.__name__}, {self.dataset_name} - #data: {self.data_num} >"


    def __del__(self):
        if (
            hasattr(self, "data_folder_h5_df")
            and self.data_folder_h5_df is not None
        ):
            self.data_folder_h5_df.close()
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(
        ...
        self.centerResize_transform = transforms.Compose(
            [
                transforms.RandomAffine(degrees=0, scale=(1.0, 1.3)),  # 随机放大
                transforms.CenterCrop(size=self.resize),  # 中心裁剪
                base_transform,
